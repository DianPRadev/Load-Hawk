...
export default function BrokerRatingsPage() {
  const { brokers, rateBroker } = useApp();
  ...
  const handleRate = () => {
    ...
    rateBroker(ratingModal, userRating, userComment);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-5">
      <div className="flex items-center justify-between animate-fade-up">
        ...
        <GoldButton size="sm" onClick={() => setRatingModal(brokers[0]?.mc || null)}>Rate a Broker</GoldButton>
      </div>
      ...
      <div className="relative animate-fade-up" style={{ animationDelay: "100ms" }}>
        ...
        <input
          ...
          className="w-full glass-input rounded-xl pl-10 pr-4 py-3 text-[13px] focus:outline-none"
        />
      </div>
      ...
      <div className="space-y-3">
        {filtered.map((b, i) => (
          <div key={b.mc} className="glass-panel rounded-2xl p-5 card-hover animate-fade-up" style={{ animationDelay: `${150 + i * 80}ms` }}>
            <div className="flex flex-col sm:flex-row sm:items-center gap-4">
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {b.badges.map(badge => (
                    <span key={badge} className={`pill-badge text-[10px] ${badge === "Avoid" ? "bg-destructive/15 text-destructive" :
                      badge === "Fast Pay" ? "bg-success/15 text-success" :
      ...
      {ratingModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={() => setRatingModal(null)}>
          ...
          <div className="relative glass-panel-heavy rounded-2xl p-6 max-w-md w-full mx-4 space-y-4 window-chrome" onClick={e => e.stopPropagation()}>
            ...
            >
              {brokers.map(b => (
                <option key={b.mc} value={b.mc}>{b.name} ({b.mc})</option>
              ))}
            </select>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full glass-input rounded-xl px-4 py-2.5 text-[13px] focus:outline-none resize-none h-20"
              />
            </div>

            <div className="flex gap-2 justify-end pt-1">
              <GoldButton variant="secondary" onClick={() => setRatingModal(null)}>Cancel</GoldButton>
              <GoldButton onClick={handleRate}>Submit Rating</GoldButton>
            </div>
          </div>
        </div>
      )}
