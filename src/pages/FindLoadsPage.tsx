    ...
    <div className="max-w-7xl mx-auto space-y-5">
      ...
      <div className="glass-panel rounded-2xl p-5 animate-fade-up">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-[11px] text-muted-foreground ml-auto">{totalCount} loads available</span>
        </div>
        {displayLoads.length > 0 ? (
          <div className="flex flex-wrap gap-2">
            {[...new Map(displayLoads.slice(0, 12).map(l => [`${l.origin}-${l.destination}`, l])).values()].slice(0, 6).map((l) => (
              <div key={l.id} className="flex items-center gap-1.5 bg-[var(--glass-highlight)] border border-[var(--glass-border)] rounded-full px-3 py-1.5 text-[12px]">
                <span>{l.origin}</span>
                <span className="text-muted-foreground">→</span>
                <span>{l.destination}</span>
                <span className="font-mono text-primary text-[11px] ml-1">${l.ratePerMile.toFixed(2)}/mi</span>
              </div>
            ))}
          </div>
        ) : (
      ...
      <div className="flex flex-wrap gap-2.5 items-center animate-fade-up" style={{ animationDelay: "100ms" }}>
        ...
        <input placeholder="Min Rate ($)" value={minRate} onChange={e => setMinRate(e.target.value.replace(/[^0-9]/g, ""))} aria-label="Minimum rate" className={`${inputClass} w-[calc(50%-5px)] sm:w-28`} />
        <input placeholder="Max Miles" value={maxDH} onChange={e => setMaxDH(e.target.value.replace(/[^0-9]/g, ""))} aria-label="Maximum miles" className={`${inputClass} w-[calc(50%-5px)] sm:w-28`} />
      ...
      {user && savedSearches.length > 0 && (
        <div className="glass-panel rounded-2xl p-4 animate-fade-up" style={{ animationDelay: "120ms" }}>
          <button
          ...
          >
            ...
            <span className="text-[11px] text-muted-foreground">({savedSearches.length})</span>
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              {savedSearches.map((s) => (
                <div
                  ...
                  className="group flex items-center gap-1.5 bg-[var(--glass-highlight)] border border-[var(--glass-border)] rounded-full px-3 py-1.5 text-[12px] cursor-pointer hover:bg-[var(--glass-active)] transition-colors"
                >
                  ...
                  {s.equipment && (
                    <span className="text-primary/70 text-[10px]">{s.equipment}</span>
                  )}
                  <button
                  ...
                  >
                    {s.alerts_enabled ? <Bell size={12} className="text-primary" /> : <BellOff size={12} />}
                  </button>
                  <button
      ...
      <div className="md:hidden space-y-3 animate-fade-up" style={{ animationDelay: "200ms" }}>
        {paginatedLoads.length === 0 ? (
          <div className="glass-panel rounded-2xl p-8 text-center text-muted-foreground text-[13px]">No loads match your filters.</div>
        ) : (
      ...
      <div className="hidden md:block glass-panel rounded-2xl overflow-hidden animate-fade-up" style={{ animationDelay: "200ms" }}>
